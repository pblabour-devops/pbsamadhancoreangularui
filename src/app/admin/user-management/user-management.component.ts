...
import { IEmpanelledPersonDetails, IOfficerTransferDetails, PasswordResetModel } from 'src/app/dashboard/dashboard-typed-models';
 ...
 error_messages = {
  ...
  getUserDetails(role: string) {
    this.appHttpRequestHandlerService
      .httpGet({ roleName: role }, 'Admin', 'getDepartmentOfficialDetailsByRoleName').pipe(takeUntil(this.ngUnsubscribe))
      .subscribe((data: any) => {
...
onUpdateProfileDetailClick(item: any, modal: any) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      this.appHttpRequestHandlerService.httpPost(item,'pbsamadhannetcoreapi.ViewModels.UpdateOfficerProfileDetailsViewModel','Admin','updateOfficerProfileDetails').pipe(takeUntil(this.ngUnsubscribe))
      .subscribe({
        next: (res: any) => {
