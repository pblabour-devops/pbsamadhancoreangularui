...
@Component({
    selector: 'app-part-iii-worker-detail-muster-roll',
    templateUrl: './part-iii-worker-detail-muster-roll.component.html',
    styleUrls: ['./part-iii-worker-detail-muster-roll.component.css'],
    standalone: false
})
export class PartIiiWorkerDetailMusterRollComponent implements OnInit {
  ...
  ngAfterViewInit() {
    this.appHttpRequestHandlerService.httpGet({ id: this.paramInfo.inspectionRefId }, "Inspection", "getForm_Factory_Part_III_MusterRoll").pipe(takeUntil(this.ngUnsubscribe))
  ...
  onSubmit(roleType: number): void {
    ...
    if (this.Input_Form.valid && this.totalCount) {
      this.appHttpRequestHandlerService.httpPost(this.Input_Form.value,"pbsamadhannetcoreapi.Models.Inspection_Form_Factory_Part_III_MusterRoll", "Inspection", "addUpdateForm_Factory_Part_III_MusterRoll")
  ...
  checkInput(event: Event) {
    ...
    if (!inputElement.value || inputElement.value.trim() === '' || inputElement.value === '-') {
      ...
      // If the value starts with "-", update it to "0"
  ...
  removeMustorRoleDetails(id) {
    this.appHttpRequestHandlerService.httpGet({ id: id }, "Inspection", "removeForm_Factory_Part_III_MusterRoll").pipe(takeUntil(this.ngUnsubscribe))
      .subscribe((data: GenericResponseTemplateModel<Inspection_Form_Factory_Part_III_MusterRoll>) => {
        this.savedData = data.responseDataModel;
        this.ngAfterViewInit();
      });
